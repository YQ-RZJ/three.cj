...
## class BgfxBloomPass
```cj
public class BgfxBloomPass <: Pass
```
Bloom post-processing pass (simple version)
...
```
Kernel size (shader loop count)

### var sigma
```cj
public var sigma: Float64
```
Gaussian sigma (controls kernel spread, used by buildKernel)

### var strength
```cj
public var strength: Float64
```
Bloom strength (blur multiplier at combine)
